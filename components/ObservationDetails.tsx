import { Image, StyleSheet, Pressable, TextProps, useWindowDimensions } from 'react-native';
...
export default function ObservationDetails({ observation, onClose }: ObservationDetailsProps) {
    ...
    <ThemedView style={{ flex: 1 }}>
      <ThemedScrollView>
        ...
        <ThemedView style={styles.headerContainer}>
          <ModalText style={styles.header}>{observation.common_name || "Observation Details"}</ModalText>
        </ThemedView>

        {/* Observation Date and Note */}
        <ThemedView style={styles.detailsContainer}>
          <ModalText>Observed on: {observation.observed_on || "Date not available"}</ModalText>
          <ModalText>

          </ModalText>
          <ModalText>{observation.description || "No notes available"}</ModalText>
        </ThemedView>

        {/* Add to Favorite Button */}
        <Pressable style={styles.button} onPress={handleAddToFavorites}>
          <ModalText>Add to Favorites</ModalText>
        </Pressable>
          ...
          {observation.photos && observation.photos.length > 0 ? (
            observation.photos.map((photo, index) => (
              <Image
                ...
                style={[styles.photo, { width: width - 32, height: width - 32 }]}
                resizeMode="cover"
              />
            ))
          ) : (
